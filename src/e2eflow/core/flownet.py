...
def _flownet_upconv(conv6_1, conv5_1, conv4_1, conv3_1, conv2, conv1=None, inputs=None,
                    channel_mult=1, full_res=False, pose_pred=False, channels=2, channels_p=12):
    m = channel_mult
    ...
    if pose_pred:
        ...
        with tf.variable_scope('pose_pred'):
            ...
            concat2 = tf.concat([conv2, flow2, meshgrid2], 1)
            pose2 = slim.conv2d(concat2, channels_p, 3, scope='pose2',
                                activation_fn=None)
            concat2_p = tf.concat([pose2, flow2, meshgrid2], 1)
            flow2_2 = slim.conv2d(concat2_p, channels, 3, scope='flow2', activation_fn=None)
            ...
            icnv3 = tf.concat([pose2, flow2_2], 1)
            cnv3 = slim.conv2d(icnv3, 16, [3, 3], stride=2, scope='cnv3', activation_fn=_leaky_relu)
            concat3 = tf.concat([conv3_1, cnv3, flow3, meshgrid3], 1)
            pose3 = slim.conv2d(concat3, channels_p, 3, scope='pose3', activation_fn=None)
            concat3_p = tf.concat([pose3, flow3, meshgrid3], 1)
            flow3_2 = slim.conv2d(concat3_p, channels, 3, scope='flow3', activation_fn=None)
            ...
            icnv4 = tf.concat([pose3, flow3_2], 1)
            cnv4 = slim.conv2d(icnv4, 16, [3, 3], stride=2, scope='cnv4', activation_fn=_leaky_relu)
            concat4 = tf.concat([conv4_1, cnv4, flow4, meshgrid4], 1)
            pose4 = slim.conv2d(concat4, channels_p, 3, scope='pose4',
                                activation_fn=None)
            concat4_p = tf.concat([pose4, flow4, meshgrid4], 1)
            flow4_2 = slim.conv2d(concat4_p, channels, 3, scope='flow4', activation_fn=None)
            ...
            icnv5 = tf.concat([pose4, flow4_2], 1)
            cnv5 = slim.conv2d(icnv5, 16, [3, 3], stride=2, scope='cnv5', activation_fn=_leaky_relu)
            concat5 = tf.concat([conv5_1, cnv5, flow5, meshgrid5], 1)
            pose5 = slim.conv2d(concat5, channels_p, 3, scope='pose5',
                                activation_fn=None)
            concat5_p = tf.concat([pose5, flow5, meshgrid5], 1)
            flow5_2 = slim.conv2d(concat5_p, channels, 3, scope='flow5', activation_fn=None)
            ...
            icnv6 = tf.concat([pose5, flow5_2], 1)
            cnv6 = slim.conv2d(icnv6, 16, [3, 3], stride=2, scope='cnv6', activation_fn=_leaky_relu)
            concat6 = tf.concat([conv6_1, cnv6, flow6, meshgrid6], 1)
            pose6 = slim.conv2d(concat6, channels_p, 3, scope='pose6',
                                activation_fn=None)
            concat6_p = tf.concat([pose6, flow6, meshgrid6], 1)
            flow6_2 = slim.conv2d(concat6_p, channels, 3, scope='flow6',
                                activation_fn=None)
